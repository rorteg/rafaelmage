...
class Uecommerce_Mundipagg_Model_Fivecreditcards extends Uecommerce_Mundipagg_Model_Standard
{
    ...
    public function assignData($data) 
    {
        ...
        if($cctype1) {
            ...
            $interestInformation[$this->_code.'_5_1'] = new Varien_Object();
            $interestInformation[$this->_code.'_5_1']->setInterest(str_replace(',','.',$interest1))->setValue(str_replace(',','.',$value1));
        }

        if($cctype2) {
            $interest2 = Mage::helper('mundipagg/installments')->getInterestForCard($parcelsNumber2 , $cctype2, $value2);
            $interestInformation[$this->_code.'_5_2'] = new Varien_Object();
            $interestInformation[$this->_code.'_5_2']->setInterest(str_replace(',','.',$interest2))->setValue(str_replace(',','.',$value2));
        }

        if($cctype3) {
            $interest3 = Mage::helper('mundipagg/installments')->getInterestForCard($parcelsNumber3 , $cctype3, $value3);
            $interestInformation[$this->_code.'_5_3'] = new Varien_Object();
            $interestInformation[$this->_code.'_5_3']->setInterest(str_replace(',','.',$interest3))->setValue(str_replace(',','.',$value3));
        }

        if($cctype4) {
            $interest4 = Mage::helper('mundipagg/installments')->getInterestForCard($parcelsNumber4 , $cctype4, $value4);
            $interestInformation[$this->_code.'_5_4'] = new Varien_Object();
            $interestInformation[$this->_code.'_5_4']->setInterest(str_replace(',','.',$interest4))->setValue(str_replace(',','.',$value4));
        }

        if($cctype5) {
            $interest5 = Mage::helper('mundipagg/installments')->getInterestForCard($parcelsNumber5 , $cctype5, $value5);
            $interestInformation[$this->_code.'_5_5'] = new Varien_Object();
            $interestInformation[$this->_code.'_5_5']->setInterest(str_replace(',','.',$interest5))->setValue(str_replace(',','.',$value5));
        }
